...
client.connect(function(err) {
	...
	client.query('SELECT *, ST_AsText(wkb_geometry), uf_compliedlist.source AS cl_source FROM uf_points2 LEFT JOIN uf_compliedlist ON uf_points2.uid=uf_compliedlist.uid;', function (err, result) {
	    if (err) throw err;
	    ...
	    for (let i = 0; i < result.rows.length; i++) {
	    	...
	    	let newProject = {
			    ...
			    Proj_Man: "TBD",
			    CD: curElmnt['cd'],
			    Access: curElmnt['project_accessibility'],
			...
			//Account for absence of geometry attributes
			curElmnt['st_astext'] == null ? newProject.Geometry = null: newProject.Geometry = parseGeometry(curElmnt['st_astext']);

		    //Write results to file
			fs.appendFile("./export_UNFUNDED_points.js", JSON.stringify(newProject) + ",\r\n", function(err){
				if (err) {
